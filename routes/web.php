...
Route::post('product/{id}/add-to-favorites', [PageController::class, 'addToFavorites'])->name('product.addToFavorites');
...
Route::group(['prefix' => 'admin', 'middleware' => 'productsLogin'], function () {
    ...
    Route::group(['prefix' => 'bill'], function () {
        ...
        Route::get('list/{status}', [BillController::class, 'getBillList'])->name('product.getBillList');
        //by laravel request
        Route::get('{id}/{status}', [BillController::class, 'updateBillStatus'])->name('product.updateBillStatus');
        //by ajax request
        Route::post('updateBillStatusAjax', [BillController::class, 'updateBillStatusAjax'])->name('product.updateBillStatusAjax');
       
        Route::delete('/bill/cancel-bill/{id}', [BillController::class, 'cancelBill'])->name('product.cancelBill');
    });
    // liên hệ
    Route::group(['prefix' => 'contacts'], function () {
        // Xem danh sách liên hệ
        Route::post('/contact', [ContactController::class, 'store'])->name('product.store');
        Route::get('list', [ContactController::class, 'getContactList'])->name('product.list');
        // Cập nhật trạng thái liên hệ
        Route::get('update-status/{id}/{status}', [ContactController::class, 'updateContactStatus'])->name('product.updateStatus');
    });
